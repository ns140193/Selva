...
public class Webservice extends General {
	...
	private void createhttpRequest(RequestMethod requestmethod) {
        ...
        private void saveXML(String reqOrRes, String data){
            String fileName="";
            String path="";
            if(reqOrRes.equals("request")){
                fileName=Report.getNewRequestFileName();
            }else if(reqOrRes.equals("response")){
                fileName=Report.getNewResponseFileName();
            }
            try{
                //if(!fileName.isBlank()){
            	if(!fileName.isEmpty()) {
                path=FilePath.getCurrentResultsPath()+File.separator+"web";
                ...
                if (location.createNewFile()) { 
                    ...
                    myWriter.write(data);
                    myWriter.close();
                    System.out.println("File created: " + location.getName());
                    }
                }
            }catch(Exception e){
                e.printStackTrace();
